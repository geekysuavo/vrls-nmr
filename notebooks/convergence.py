...
def _():
    ...
    return F, Signal, plt, torch, vrls, vrls_mf
...
def _(ground_truth, sched, torch):
    ...
    ids = ids.cuda()
    y = y.cuda()
    return ids, n, niter_fast, niter_slow, sigma, tau, xi, y


@app.cell
def _(ids, n, niter_fast, niter_slow, tau, vrls, vrls_mf, xi, y):
    (mu, gamma_diag, yhat, sigma_diag) = vrls(y, ids, tau, xi, n, niter_slow)
    (mu_fast, gamma_fast, _, _) = vrls_mf(y, ids, tau, xi, n, niter_fast)
...
def _(
    ...
    plt.plot(mu.real.cpu(), label=f"VRLS ({niter_slow} iterations)")
    plt.xlim((800, 1000))
    plt.plot(mu_fast.real.cpu(), label=f"FMF-VRLS ({niter_fast} iterations)")
    plt.grid(alpha=0.2)
    plt.legend()
