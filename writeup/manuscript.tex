...
This section begins by reintroducing \emph{iteratively reweighted least
squares} (IRLS, \cite{daubechies:cpam2010,kazimierczuk:jmr2011}) as a
classical method for solving sparse recovery problems in compressed sensing.
The machinery of Bayesian inference is then brought to bear in order to
demonstrate that IRLS naturally arises as the \emph{maximum a posteriori}
(MAP) estimator of a particular probability distribution. The additional
machinery of variational approximation is then employed to construct
\emph{variationally reweighted least squares} (VRLS), which is functionally
similar to IRLS but yields posterior mean and variance estimates.
Finally, ANS is developed around VRLS.
...
\begin{aligned}
 \underset{\m{x}}{\text{minimize}} &\; \|\m{x}\|_1 \\
 \text{such that} &\; \| \m{y} - \m{A} \m{x} \|_2^2 \le m \sigma^2
\end{aligned}
\label{eq:l1_ineq}
\end{equation}
...
results in a simple minimization algorithm (IRLS) composed of two
steps per iteration:
\begin{enumerate}
 \item \emph{(Least Squares)}
  $\m{x} \gets \m{W}^{-1} \m{A}^\top
  \left( \m{A} \m{W}^{-1} \m{A}^\top \right)^{-1} \m{y}$
 \item \emph{(Reweighting)}
  $\forall i : W_{ii} \gets |x_i|^{-1}$
\end{enumerate}
...
\label{eq:family}
\mathcal{Q} = \{
 q \in \mathcal{Q}_0 \mid q(\m{x}, \m{w}) = q_x(\m{x}) \, q_w(\m{w})
\}
\end{equation}
where $\mathcal{Q}_0$ denotes the set of all possible distributions,
i.e.~positive functions of $\m{x}$ and $\m{w}$ that integrate to one.
...
the updates to $\m{\nu}$ only require the diagonal entries of $\m{\Gamma}$,
and the posterior predictive variances of $\m{\hat y}$ only require the
diagonal entries of $\m{\Phi} \m{\Gamma} \m{\Phi}^\top$. Both of these
benefit from kernelization, requiring only $\mathcal{O}(m^2 n)$ operations.
For example, any diagonal entry $\Gamma_{ii}$ may be computed using the
...
\caption{%
 (a) Normalized mean-squared error reductions
 $\Delta E_{\mathrm{unif,VRLS}}^{\mathrm{ANS}}$ in the
...
a fully factorized \emph{mean-field} family,\footnote{%
 Note that $\mathcal{Q}_{\mathrm{MF}}$ is more restrictive than $\mathcal{Q}$,
 i.e.~$\mathcal{Q}_0 \supset \mathcal{Q} \supset \mathcal{Q}_{\mathrm{MF}}$.
}
\begin{equation}
\mathcal{Q}_{\mathrm{MF}} = \{
 q \in \mathcal{Q}_0 \mid
 q(\m{x}, \m{w}) = {\textstyle\prod_i} q_x(x_i) \, q_w(w_i)
\}
\end{equation}
...
\end{equation}
and we need only the marginal variances, i.e.~the diagonal entries
...
\caption{%
 ...
 and ANS simulations of the same signal.
...
FMF-VRLS, and IST-S \cite{stern:jmr2007}. Normalized mean squared errors
(NMSE) of the form,
\begin{equation}
E(\m{x}, \m{\hat x}) =
 \frac{\| \m{x} - \m{\hat x} \|_2^2}{\| \m{x} \|_2^2}
\end{equation}
where $\m{x}$ denotes the ground truth spectrum and $\m{\hat x}$ denotes
the spectral estimate, were computed for each combination of schedule and
reconstruction algorithm. NMSE values were computed for entire spectra as
well as the spectral regions known to contain only signal or noise.
Error differences ($\Delta$NMSE) of the form
\begin{equation}
\Delta E_{\mathrm{sched,algo}}^{\mathrm{ANS}} =
 E(\m{x}, \m{\hat x}_{\mathrm{sched,algo}}) -
 E(\m{x}, \m{\hat x}_{\mathrm{ANS,algo}})
\end{equation}
were computed to compare errors of ANS to errors of \emph{a priori}
schedules for the same algorithm, and relative errors of the form
\begin{equation}
\Delta E_{\mathrm{sched,algo}}^{\mathrm{VRLS}} =
 E(\m{x}, \m{\hat x}_{\mathrm{sched,algo}}) -
 E(\m{x}, \m{\hat x}_{\mathrm{sched,VRLS}})
\end{equation}
were computed to compare errors of VRLS to errors of other
reconstruction algorithms for the same schedule. For IST-S, $\m{\hat x}$
is the point estimate, essentially $\m{\hat x}_{\mathrm{MAP}}$. For VRLS
and FMF-VRLS, $\m{\hat x}$ is the mean estimate $\m{\mu}$.
...
When posterior variance estimates are not required, FMF-VRLS can provide
mean estimates $\m{\mu}$ of equal quality to VRLS without requiring the
storage or inversion of matrices. However, the matrix-free advantage of
FMF-VRLS comes with disadvantages. Because FMF-VRLS neglects posterior
correlations between spectral points, it dramatically under-estimates
frequency domain posterior variance, and completely fails to usefully
estimate time-domain posterior variance. Moreover, it often requires an
order of magnitude more iterations than VRLS to converge.
Figure \ref{fig:time} summarizes this effect: for sufficiently small
$m$ (high undersampling), VRLS requires less time than FMF-VRLS to
converge to an equivalent or better mean estimate.

\subsection{Active nonuniform sampling simulations}
\label{subsec:ansresults}
Comparing the VRLS NMSE distributions from ANS and \emph{a priori} NUS
schedules reveals largely equivalent performance in the aggregate: all
reconstructions have NMSE less than one, indicating reasonably good fits.
Overall, uniform and exponential schedules have lower signal-region NMSE,
whereas ANS and Poisson-gap schedules have lower noise-region NMSE.

Additional insights are gained by examining distributions of $\Delta$NMSE
values where the only difference is the sampling schedule (Figures
\ref{fig:errors}, \ref{fig:ans}). The degree to which ANS reduces VRLS
reconstruction error in noise-only spectral regions, relative to uniform
\emph{a priori} NUS, is dependent on the measurement noise $\sigma$ and
the final undersampling rate $m/n$ (Figure \ref{fig:errors}a). This
dependence is most apparent when comparing ANS to uniform NUS schedules,
but can be observed to varynig degree in envelope-matched exponential
schedules and Poisson-gap schedules (Figure \ref{fig:errors}b). When
acquiring noisier data, the information gained for each new grid point
is lower, reducing the advantage of ANS. Likewise, in less aggressively
undersampled experiments, the variability of reconstructions due to the
choice of schedule is diminished. By leveraging the uncertainty estimates
and variational free energies computed by VRLS to form an early stopping
criterion, we may therefore limit ANS to this ideal undersampling regime
of $m \ll n$ while controlling reconstruction error. Detailed histograms
of $\Delta$NMSE are shown in Figures S1 in the \nameref{app:si}.

To illustrate a concrete result from the Figure \ref{fig:errors}a metrics,
Figure \ref{fig:ans} compares VRLS reconstructions of the same signal
using ANS and uniform \emph{a priori} NUS. Near true resonances, ANS
errors are the same or slightly increased relative to \emph{a priori} NUS,
...
dramatically reduced relative to \emph{a priori} NUS, which tends to produce
sharper artifacts more randomly within the spectral window. These trends
are mirrored in the posterior variance estimates from ANS and
\emph{a priori} NUS, demonstrating that ANS preferentially selects
grid points that inform VRLS about where the signal \emph{is not present}
in the spectrum, as opposed to where it is. Viewed from the lens of solving
the highly underdetermined linear system \eqref{eq:signalmodel}, such a
...
exhibiting less pronounced, broader minima. Figure S3 in the
\nameref{app:si} highlights a single representative ANS result for the
same signal used for Figures \ref{fig:sched} and \ref{fig:psf}.

Finally, it is important to note that the schedules produced by ANS are
less ``universal'' than other \emph{a priori} sampling schedules, as they
have been specifically selected to reduce VRLS uncertainty. As a consequence,
improvements in reconstruction quality are not guaranteed when a different
reconstruction algorithm is applied to the same data. Figure S2 in the
\nameref{app:si} illustrates this behavior for FMF-VRLS and IST-S. This
decreased robustness was also observed in the case of beat-matched NUS
\cite{schuyler:jbnmr2011}, although for slightly different reasons. Further
analysis is needed to understand whether this observation also applies to
ANS data reconstructed using inferred-basis signal models that have fewer
degrees of freedom than fixed-basis models like IRLS and IST.
...
this is more than an aesthetic trait. In addition, while expected
information gain is a common utility function in active learning,
other utility functions may usefully bias ANS towards acquiring better
data in specific cases like low signal-to-noise or high dynamic range.
Perhaps most obviously, further work is needed to extend VRLS to
multidimensional data where the advantages of NUS are greater. Doing so
is largely an engineering exercise that will require careful indexing
and multi-complex FFTs, as nothing in VRLS precludes its generalization
...
the advantages of (i) further reducing acquisition time by leveraging
prior information, (ii) reducing the number of uncertain degrees of freedom
and (iii) providing spectroscopists with richer and more immediate
experimental results. Indeed, further work is needed to develop Bayesian
NMR signal models that make full use of available prior knowledge while
also admitting active learning.
