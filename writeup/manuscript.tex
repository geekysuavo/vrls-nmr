...
\includegraphics[width=0.48\textwidth]{example-image-a}
\caption{%
 Reconstruction errors produced from static NUS and ANS simulations
 of signals with randomly generated parameters.
}
\label{fig:ans}
\end{figure}
...
\item Look at masked MSE around peaks only
\item Compare to uniform, exponential, Poisson-gap
\end{itemize}
% end notes to delete
...
active learning on NMR spectrometers.\footnote{%
 ...
 However, their model required minutes to propose new grid points
 and suffers from all the shortcomings of deep learning (black box
 inscrutability, unclear out-of-domain generalization, inference-time
 non-determinism, etc.).
