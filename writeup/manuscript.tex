...
from 5\%-sparse quadrature measurements ($m=51, n=1024$). Synthetic data
were generated to test ANS across a broad variety of signal parameters.
All simulated signals have the form
\begin{equation}
...
frequencies $\{\omega_c\}$ drawn from a uniform distribution, decay rates
$\{\rho_c\}$ drawn from a scaled chi-squared distribution, and phase
shifts $\{\theta_c\}$ drawn from a normal distribution. The noise
$\epsilon$ is drawn from a normal distribution with variance
$\sigma^2 = 1/\tau$.
...
\begin{itemize}
 \item Reconstruct my 5\% NUS 1H-15N HSQC
 \item Compare quality/convergence of VRLS to VRLS-FMF
 \item Analyze runtime as $m$ increases for selected fixed $n$
 \item Simulate ANS using synthetic data
 \begin{itemize}
  \item Look at PDFs/PSFs from ANS runs on same signal
  \item Schedule characteristics depend on $n$-heuristic
  \item Look at $\ell_2$ error of spectral estimates
  \item Manually exponentially bias the variance?
  \item Post-hoc variance computation fails in ANS
 \end{itemize}
 \item VRLS convex, globally convergent
 \item $\Gamma_{ii}$ useful as an ``artifact meter''
\end{itemize}
% end notes to delete
...
faced by its alternatives like sparse Bayesian learning
...
implementation are expected to further improve runtime as $m$ increases.
...
this is more than an aesthetic flaw. Perhaps most obviously, further work
...
desiderata of ANS and consequently enable model-based active learning
on NMR spectrometers. However, it remains limited in many important
aspects. Principal among these limitations its its continued use of a
fixed-basis signal model \eqref{eq:signalmodel}, which discards
