...
\title{VRLS for NUS NMR}
\author{Bradley Worley}
\maketitle

\begin{abstract}
Notes on an efficient iteratively reweighted least squares algorithm that
has been purpose-tuned for active nonuniform sampling of nuclear magnetic
resonance data.
\end{abstract}
...
We begin by bounding the error term in $F(\m{\eta})$,
\begin{equation}
...
After substituting this bound into $F(\m{\eta})$ to obtain a new
function $\hat F(\m{\eta})$,
\begin{equation}
\begin{aligned}
\hat F(\m{\eta}) &=
